...
const GymSelecotr = ({ label, name, value, onChange, placeholder }) => {
  ...
  const handleSelect = ({ idx, item }) => {
    setSelectIdx(idx);
    // 부모 컴포넌트에 선택된 체육관 정보 전달
    if (onChange) {
      onChange({ target: { name: name, value: item.gym } });
    }
  };
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="nav flex">
        <div
          className={
            "cursor-pointer rounded-l-sm " +
            (newGym ? unSelectClass : selectClass)
          }
        ...
        <div
          className={
            ...
            (newGym ? selectClass : unSelectClass)
          }
          onClick={() => setNewGym(true)}
        >
          신규
        </div>
      </div>
      ...
      <div
        className={`select-gym pt-[1rem] flex flex-col gap-2 ${
          newGym ? "hidden" : ""
        }`}
      >
        <div className="px-2">
          ...
          <ListSelector
            ...
            Template={({ item }) => (
              <div className="w-full px-2 py-2">
                <div className="title font-bold">{item.gym}</div>
                <div className="address text-[.7rem]">{item.gymAddress}</div>
              </div>
            )}
          />
        </div>
      ...
      >
        ...
        <div className="flex justify-end">
          <div
            ...
            className="w-[6rem] cursor-pointer text-center rounded-sm border"
          >
            주소 검색
          </div>
        </div>
